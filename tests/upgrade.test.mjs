...
test("plans an older-version upgrade without writing recovery or managed files", () => {
  ...
  try {
    ...
    assert.equal(plan.toVersion, "1.0.4");
...
test("upgrades unchanged managed files, preserves original merges, and restores obsolete user files", () => {
  ...
  try {
    ...
    assert.equal(manifest.package.version, "1.0.4");
    ...
    assert.match(formatProjectUpgradeResult(result), /0\.2\.0 -> 1\.0\.4/);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("upgrade refresh rebuilds modules, paths, and task allowlists from Gradle", () => {
  ...
  try {
    ...
    assert.equal(result.status, "upgraded");
    assert.equal(result.doctor.ok, true);
    assert.equal(result.fromVersion, "1.0.4");
    assert.equal(result.toVersion, "1.0.4");
    assert.deepEqual(
