...
for _step in 1 2 3 4 5 6 7 8; do
    ...
    case "$state" in
        ...
        REVIEWING)
            ...
            reviewer_prompt="Use \$scheduled-quality-reviewer with $task_id. This is fresh, non-interactive, read-only review attempt $review_attempt for sealed review cycle $review_cycle. Work within the step budget. Run exactly ./scripts/automation/status.sh $task_id once; its JSON already includes baseline, RED, ready, latest gate, current diff SHA, the sealed-SHA comparison, and runtime.effectiveWorktreeAllowlist, so do not browse the external evidence directory. Then inspect the approved contract and run exactly git diff, excluding the control file and every exact path in that effective list from review findings. Submit exactly one decision. Do not browse generated build reports, list the whole repository, retry denied command variants, or run Gradle commands separately before deciding: submit-review.sh performs and records focused and full unit tests when unitTestsEnabled is true, always runs assembleDebug, and runs Android lint when lintEnabled is true for APPROVED. Reserve a tool call for submit-review.sh. Never edit, commit, merge, or push."
