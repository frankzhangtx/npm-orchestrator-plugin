...
`@frankzhang2026/opencode-android-orchestrator@1.0.4`. Pin the exact version and
...
| --- | --- | --- |
| No orchestrator files or manifest | `npx @frankzhang2026/opencode-android-orchestrator@1.0.4 init .` | Normal new installation; all runtime-detected Android modules and registered debug verification tasks are discovered automatically. |
...
| Healthy `1.0.0` through `1.0.3` installation | Stop the queue service, finish or abort retained workspaces, then run the fixed `1.0.4` `upgrade`. | Pending inbox contracts remain durable. Version 1.0.4 resumes the queue and starts or wakes its service after a newly approved contract is enqueued, while preserving existing scheduling and failure state. |
...
```sh
npx --yes --registry=https://registry.npmjs.org/ \
  @frankzhang2026/opencode-android-orchestrator@1.0.4 upgrade . \
