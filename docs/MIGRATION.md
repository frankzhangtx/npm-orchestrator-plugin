...
`@frankzhang2026/opencode-android-orchestrator@0.4.0`. Pin the exact version and
...
| --- | --- | --- |
| No orchestrator files or manifest | `npx @frankzhang2026/opencode-android-orchestrator@0.4.0 init .` | Normal new installation; all detected Android modules are enabled by default. |
...
| Healthy `0.2.0` or `0.3.0` manifest-managed installation | Run the fixed `0.4.0` doctor, then `upgrade`. | A missing module-scope field is treated as `primary`; an existing scope is preserved unless explicitly changed. |
...
`0.4.0 init`. The merger deliberately rejects a different version of the same
