...
`@frankzhang2026/opencode-android-orchestrator@0.5.0`. Pin the exact version and
...
| --- | --- | --- |
| No orchestrator files or manifest | `npx @frankzhang2026/opencode-android-orchestrator@0.5.0 init .` | Normal new installation; all detected Android modules and registered debug verification tasks are discovered automatically. |
...
| Healthy `0.2.0`, `0.3.0`, or `0.4.0` manifest-managed installation | Run the fixed `0.5.0` doctor, then `upgrade`. | A missing module-scope field is treated as `primary`; existing module scope and verification tasks are preserved unless explicitly changed. |
...
`0.5.0 init`. The merger deliberately rejects a different version of the same
