...
`@frankzhang2026/opencode-android-orchestrator@0.4.0`. The lifecycle foundation
completed the real OpenCode `1.14.22` and `1.15.13` release matrix in `0.2.0`;
`0.4.0` retains that compatibility boundary.
...
reason `0.4.0` exposes only `android_orchestrator_status` and
